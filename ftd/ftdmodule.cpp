...
void Module::open(const string& desc) {
	...
	if(mIsOpen == false)
		throw runtime_error("Failed open ftd");
}

void Module::initialize(const ChannelConfig& conf) {
	init(conf);
}

void Module::read(uint8_t* buff, uint32_t size) {
	uint32_t retCount;
	deviceRead(size, buff, retCount, 0x3);
}

void Module::write(uint8_t* buff, uint32_t size) {
	uint32_t retCount;
	...
	if(options & I2C_TRANSFER_OPTIONS_FAST_TRANSFER)
		...
		for(i = 0; i < sizeToTransfer; ++i) {
			...
			if(i < sizeToTransfer - 1)
				...
				flag = (options & I2C_TRANSFER_OPTIONS_NACK_LAST_BYTE) == 0;
			}
...
void Module::fastRead(uint32_t sizeToTransfer, uint8_t* buffer, uint32_t& sizeTransferred, uint32_t options) {
	...
	while(j < bitsToTransfer) {
		...
		if(options & I2C_TRANSFER_OPTIONS_FAST_TRANSFER_BYTES) {
			...
			outBuffer[i++] = MPSSE_CMD_SET_DATA_BITS_LOWBYTE;
			outBuffer[i++] = VALUE_SCLLOW_SDALOW ;
			//GB set SDA to in so that following data lockout has no effect, ack/nak driven by the other I/O line on SDA
